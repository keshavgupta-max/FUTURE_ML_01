...
class SalesDataLoader:
    ...
    def load_clean_resample(self) -> pd.DataFrame:
        ...
        # 1. Clean and parse text dates into readable timestamps
        df[self.date_col] = pd.to_datetime(df[self.date_col], errors='coerce')
        df = df.dropna(subset=[self.date_col, self.target_col])
        
        # 2. Select the core columns the business cares about
        # This keeps Sales, Profit, Discount, Category, and Region intact
        important_cols = [self.date_col, self.target_col, 'Profit', 'Discount', 'Category', 'Region']
        df = df[important_cols]
        
        # 3. Group by Date, Category, and Region so we don't lose location and product data
        daily_grouped = df.groupby([self.date_col, 'Category', 'Region']).agg({
            self.target_col: 'sum',   # Total Sales volume
            'Profit': 'sum',          # Total Profit margins
            'Discount': 'mean'        # Average Discount offered
        }).reset_index()
        
        # 4. Set date as index for timeline processing
        daily_grouped.set_index(self.date_col, inplace=True)
        
        # 5. Resample to Weekly intervals per Category/Region group, filling empty rows with 0
